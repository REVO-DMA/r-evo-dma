...
async function deployWeb() {
    ...
    if (environment === "prod") destination += "api.evodma.com";
    else if (environment === "dev") destination += "dev-api.evodma.com";

    console.log("Stopping API...");
    await sshExec(`pm2 stop ${destination}/main.js`);
    ...
    console.log("Installing API dependencies with Yarn...");
    await sshExec(`cd ${destination} && yarn install --frozen-lockfile --prod`);

    console.log("Starting API...");
    await sshExec(`pm2 start ${destination}/main.js -i max`);
