...
namespace vmmsharp
{
    ...
    /// Must be disposed!
    /// </summary>
    public sealed class SCATTER_HANDLE(Dictionary<ulong, SCATTER_PAGE> scatters, IntPtr pppMEMs) : IDisposable
    {
        private readonly IntPtr _pppMEMs = pppMEMs;

        /// <summary>
        /// Scatter Read Results.
        /// </summary>
        public IReadOnlyDictionary<ulong, SCATTER_PAGE> Scatters { get; } = scatters;
    ...
    /// </summary>
    public readonly struct SCATTER_PAGE(IntPtr pb)
    {
        private readonly IntPtr _pb = pb;

        /// <summary>
        /// Page for this scatter read entry.
        /// </summary>
        public unsafe ReadOnlySpan<byte> Page => new(_pb.ToPointer(), 0x1000);
    }
    ...
    public sealed partial class Vmm : IDisposable
    {
        ...
        public unsafe SCATTER_HANDLE MemReadScatterCustom(uint pid, uint flags, ulong[] qwA)
        {
            ...
            Dictionary<ulong, SCATTER_PAGE> MEMs = new(qwA.Length);
            for (int i = 0; i < qwA.Length; i++)
            {
                var pMEM = ppMEMs[i];
                if (pMEM->f)
