...
namespace arena_dma_backend.Unity
{
    public sealed class Transform
    {
        ...
        public Transform(ulong transformInternal, TransformType transformType = TransformType.Normal, bool initCached = false, bool posCached = false)
        {
            _transformType = transformType;
            _initCached = initCached;
            _posCached = posCached;

            var hierarchy = Memory.ReadPtr(transformInternal + UnityOffsets.TransformInternal.Hierarchy, _initCached);
            IndicesAddr = Memory.ReadPtr(hierarchy + UnityOffsets.TransformHierarchy.Indices, _initCached);
            VerticesAddr = Memory.ReadPtr(hierarchy + UnityOffsets.TransformHierarchy.Vertices, _initCached);
        ...
        public Vector3 GetPosition(SharedContainer<Vector128<float>> vertices = null)
        {
            if (vertices == null)
                vertices = ReadVertices(VerticesAddr, 3 * HierarchyIndex + 3, _posCached); // Reference v9/v10 in IDA
        ...
        public static Vector3 GetPositionInternal(int[] indices, SharedContainer<Vector128<float>> verticesIn, int hierarchyIndex, TransformType transformType = TransformType.Normal)
        {
            ...
            ReadOnlySpan<Vector128<float>> vertices = verticesIn.Span;
            ...
            while (index >= 0)
            {
                ...
                else if (index > (vertices.Length / 3) - 1)
                    break;
            ...
            // return result
            Vector3 pos = result.AsVector3();
            if (pos == Vector3.Zero || !pos.IsNormal())
                throw new Exception("Invalid Position!");

            return pos;
        }

        public Quaternion GetRotation(SharedContainer<Vector128<float>> vertices = null)
        {
            if (vertices == null)
                vertices = ReadVertices(VerticesAddr, 3 * HierarchyIndex + 3, _posCached); // Reference v9/v10 in IDA
        ...
        private static int[] ReadIndices(ulong address, int count, bool useCache)
        {
            ...
            Memory.ReadBuffer(address, buf.AsSpan());
        ...
        /// </summary>
        private static SharedContainer<Vector128<float>> ReadVertices(ulong address, int count, bool useCache)
        {
            SharedContainer<Vector128<float>> vertices = new(count);
            try
            {
                Memory.ReadBuffer(address, vertices.Span, useCache);
                return vertices;
            }
            catch
            {
