...
{
    public sealed class EFTScatterRound : ScatterReadRound
    {
        public EFTScatterRound(Dictionary<int, Dictionary<int, IScatterEntry>> results, bool useCache) : base(results, useCache)
        {
        }
        ...
        /// <returns></returns>
        public override EFTScatterEntry<T> AddEntry<T>(int index, int id, object addr, object size = null, uint offset = 0x0)
        {
            var entry = new EFTScatterEntry<T>()
            {
                Index = index,
                Id = id,
                Addr = addr,
