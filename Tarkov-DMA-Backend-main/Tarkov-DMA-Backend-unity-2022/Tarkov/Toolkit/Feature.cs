...
namespace Tarkov_DMA_Backend.Tarkov.Toolkit
{
    public abstract class Feature
    {
        ...
        public Feature(int delayMs, string id, params object[] parameters)
        {
            ID = id;
        ...
        public void TryRun(ref List<ScatterWriteEntry> writes, Player localPlayer)
        {
            try
            {
                if (RunImmediately)
                {
                    Run(ref writes, localPlayer, _parameters);
        ...
        public abstract void Run(ref List<ScatterWriteEntry> writes, Player localPlayer, params object[] parameters);
    }
}
