...
namespace AssetFactory
{
	...
	bool LoadBundle(uintptr_t assetBundle, uintptr_t shaderName, uintptr_t shaderTypeObject)
	{
        ...
        if (!LoadedBundle)
        {
            LOG(xorstr_("[LoadBundle] -> Unable to load asset bundle from ") + Debug::AddrToHex(assetBundle));
            return false;
        }
        else
            LOG(xorstr_("[LoadBundle] -> Asset Bundle created at ") + Debug::AddrToHex(LoadedBundle));

		LoadedShader = Game::Call(AssetBundle_LoadAsset, LoadedBundle, shaderName, shaderTypeObject);
        if (!LoadedShader)
        {
            LOG(xorstr_("[LoadBundle] -> Unable to load shader from name at ") + Debug::AddrToHex(shaderName) + Debug::S(xorstr_(" from asset bundle at ")) + Debug::AddrToHex(LoadedBundle));
            return false;
        }
        else
            LOG(xorstr_("[LoadBundle] -> Shader created at ") + Debug::AddrToHex(LoadedShader));
    ...
    int ShaderPropertyToID(uintptr_t propertyName)
    {
        ...
        auto id = static_cast<int>(Game::Call(Shader_PropertyToID, propertyName));
