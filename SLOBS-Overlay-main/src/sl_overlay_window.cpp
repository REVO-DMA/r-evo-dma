...
bool overlay_window::create_window()
{
	if (overlay_hwnd == nullptr && ready_to_create_overlay())
	{
		...
		DWORD const dwStyleEx = WS_EX_LAYERED | WS_EX_TOPMOST | WS_EX_NOACTIVATE | WS_EX_TRANSPARENT | 0x0000080;
		//| 0x40000000
		//| 0x80000000
		//| 0x20000000;
		...
		if (overlay_hwnd)
		{
			...
			// Fullscreen
			//MoveWindow(overlay_hwnd, 0, 0, GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN), 0);
