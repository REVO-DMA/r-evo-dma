...
namespace vmmsharp
{
    ...
    public sealed class LeechCore : IDisposable
    {
        ...
        public static unsafe LeechCore CreateFromConfig(ref LC_CONFIG pLcCreateConfig, out LC_CONFIG_ERRORINFO ConfigErrorInfo)
        {
            ...
            IntPtr hLC = Lci.LcCreateEx(ref pLcCreateConfig, out nint pLcErrorInfo);
        ...
        public LeechCore(string strDevice, string strRemote, uint dwVerbosityFlags, ulong paMax)
        {
            LC_CONFIG cfg = new()
            {
                ...
                szDevice = strDevice,
                szRemote = strRemote,
                dwPrintfVerbosity = dwVerbosityFlags,
                paMax = paMax
            };
            IntPtr hLC = Lci.LcCreate(ref cfg);
            if (hLC == IntPtr.Zero)
        ...
        {
            if (!vmm.ConfigGet(Vmm.OPT_CORE_LEECHCORE_HANDLE, out ulong pqwValue))
            {
        ...
        public MEM_SCATTER[] ReadScatter(params ulong[] pas)
        {
            ...
            if (!Lci.LcAllocScatter1((uint)pas.Length, out nint pppMEMs))
            {
                return null;
            }
        ...
        public unsafe bool Write(ulong pa, byte[] data) => WriteArray(pa, data);
        ...
        public void WriteScatter(ref MEM_SCATTER[] MEMs)
        {
            ...
            if (!Lci.LcAllocScatter1((uint)MEMs.Length, out nint pppMEMs))
            {
                return;
            }
            vappMEMs = pppMEMs.ToInt64();
        ...
        {
            if (this.Command(LC_CMD_MEMMAP_GET, null, out byte[] bMemMap) && (bMemMap.Length > 0))
            {
        ...
        {
            return this.Command(LC_CMD_MEMMAP_SET, Encoding.UTF8.GetBytes(sMemMap), out _);
        }
    }
    ...
    public sealed partial class Vmm : IDisposable
    {
        ...
        {
            int cbERROR_INFO = Marshal.SizeOf<Lci.LC_CONFIG_ERRORINFO>();
            IntPtr hVMM = Vmmi.VMMDLL_InitializeEx(args.Length, args, out nint pLcErrorInfo);
        ...
        {
            this.hVMM = Initialize(out _, args);
        }
        ...
        public unsafe MEM_SCATTER[] MemReadScatter(uint pid, uint flags, params ulong[] qwA)
        {
            ...
            long vappMEMs, vapMEM;
            IntPtr pMEM, pMEM_qwA;
            if (!Lci.LcAllocScatter1((uint)qwA.Length, out nint pppMEMs))
            {
                return null;
            }
            vappMEMs = pppMEMs.ToInt64();
        ...
        /// <param name="value"><typeparamref name="T"/> Value to write.</param>
        /// <returns>True if write successful, otherwise False.</returns>
        public unsafe bool MemWriteStruct<T>(uint pid, ulong qwA, T value) where T : unmanaged
        {
            uint cb = (uint)sizeof(T);
            byte* pb = (byte*)&value;
        ...
        /// <param name="flags">VMM Flags.</param>
        /// <returns>True if write successful, otherwise False.</returns>
        public unsafe bool MemWriteArray<T>(uint pid, ulong qwA, T[] data) where T : unmanaged
        {
            uint cb = (uint)sizeof(T) * (uint)data.Length;
            fixed (T* pb = data)
            {
        ...
        {
            // checks:
            if (search == null || search.Length == 0 || search.Length > 16) { return Array.Empty<ulong>(); }
            ...
            {
                if (search[i].pbSearch == null || search[i].pbSearch.Length == 0 || search[i].pbSearch.Length > 32) { return Array.Empty<ulong>(); }
                if ((search[i].pbSearchSkipMask != null) && (search[i].pbSearchSkipMask.Length > search[i].pbSearch.Length)) { return Array.Empty<ulong>(); }
            ...
            Vmmi.VMMDLL_MEM_SEARCH_CONTEXT ctx = new()
            {
            ...
            if (!Vmmi.VMMDLL_MemSearch(hVMM, pid, ref ctx, out nint ppva, out uint pcva)) { return Array.Empty<ulong>(); }
        ...
        public unsafe IMAGE_SECTION_HEADER[] ProcessGetSections(uint pid, string wszModule)
        {
            ...
            result = Vmmi.VMMDLL_ProcessGetSections(hVMM, pid, wszModule, null, 0, out uint cData);
        ...
        public unsafe MAP_PTEENTRY[] Map_GetPte(uint pid, bool fIdentifyModules = true)
        {
            ...
            MAP_PTEENTRY[] m = Array.Empty<MAP_PTEENTRY>();
            if (!Vmmi.VMMDLL_Map_GetPte(hVMM, pid, fIdentifyModules, out nint pMap)) { goto fail; }
        ...
        public unsafe MAP_VADENTRY[] Map_GetVad(uint pid, bool fIdentifyModules = true)
        {
            ...
            MAP_VADENTRY[] m = Array.Empty<MAP_VADENTRY>();
            if (!Vmmi.VMMDLL_Map_GetVad(hVMM, pid, fIdentifyModules, out nint pMap)) { goto fail; }
        ...
        public unsafe MAP_VADEXENTRY[] Map_GetVadEx(uint pid, uint oPages, uint cPages)
        {
            ...
            MAP_VADEXENTRY[] m = Array.Empty<MAP_VADEXENTRY>();
            if (!Vmmi.VMMDLL_Map_GetVadEx(hVMM, pid, oPages, cPages, out nint pMap)) { goto fail; }
        ...
        public unsafe MAP_MODULEENTRY[] Map_GetModule(uint pid, bool fExtendedInfo)
        {
            ...
            MAP_MODULEENTRY[] m = Array.Empty<MAP_MODULEENTRY>();
            uint flags = fExtendedInfo ? (uint)0xff : 0;
            if (!Vmmi.VMMDLL_Map_GetModule(hVMM, pid, out nint pMap, flags)) { goto fail; }
        ...
        {
            MAP_MODULEENTRY e = new();
            if (!Vmmi.VMMDLL_Map_GetModuleFromName(hVMM, pid, wszModuleName, out nint pMap, 0)) { goto fail; }
        ...
        public unsafe MAP_UNLOADEDMODULEENTRY[] Map_GetUnloadedModule(uint pid)
        {
            ...
            MAP_UNLOADEDMODULEENTRY[] m = Array.Empty<MAP_UNLOADEDMODULEENTRY>();
            if (!Vmmi.VMMDLL_Map_GetUnloadedModule(hVMM, pid, out nint pMap)) { goto fail; }
        ...
        public unsafe MAP_EATENTRY[] Map_GetEAT(uint pid, string wszModule, out MAP_EATINFO EatInfo)
        {
            ...
            MAP_EATENTRY[] m = Array.Empty<MAP_EATENTRY>();

            if (!Vmmi.VMMDLL_Map_GetEAT(hVMM, pid, wszModule, out nint pMap))
                goto fail;

            Vmmi.VMMDLL_MAP_EAT* nM = (Vmmi.VMMDLL_MAP_EAT*)pMap;
            ...
            for (int i = 0; i < nM->cMap; i++)
            {
                Vmmi.VMMDLL_MAP_EATENTRY* n = (Vmmi.VMMDLL_MAP_EATENTRY*)(IntPtr)(pMap.ToInt64() + cbMAP + i * cbENTRY);
                MAP_EATENTRY e;
                e.vaFunction = n->vaFunction;
                e.dwOrdinal = n->dwOrdinal;
                e.oFunctionsArray = n->oFunctionsArray;
                e.oNamesArray = n->oNamesArray;
                e.wszFunction = Marshal.PtrToStringUni(n->wszFunction);
                e.wszForwardedFunction = Marshal.PtrToStringUni(n->wszForwardedFunction);
                m[i] = e;
            }
        ...
        public unsafe MAP_IATENTRY[] Map_GetIAT(uint pid, string wszModule)
        {
            ...
            MAP_IATENTRY[] m = Array.Empty<MAP_IATENTRY>();
            if (!Vmmi.VMMDLL_Map_GetIAT(hVMM, pid, wszModule, out nint pMap)) { goto fail; }
        ...
        public unsafe MAP_HEAP Map_GetHeap(uint pid)
        {
            ...
            MAP_HEAP Heap;
            Heap.heaps = Array.Empty<MAP_HEAPENTRY>();
            Heap.segments = Array.Empty<MAP_HEAPSEGMENTENTRY>();
            if (!Vmmi.VMMDLL_Map_GetHeap(hVMM, pid, out nint pMap)) { goto fail; }
        ...
        public unsafe MAP_HEAPALLOCENTRY[] Map_GetHeapAlloc(uint pid, ulong vaHeapOrHeapNum)
        {
            ...
            if (!Vmmi.VMMDLL_Map_GetHeapAlloc(hVMM, pid, vaHeapOrHeapNum, out nint pHeapAllocMap)) { return Array.Empty<MAP_HEAPALLOCENTRY>(); }
            ...
            if (nM.dwVersion != Vmmi.VMMDLL_MAP_HEAPALLOC_VERSION)
            {
                ...
                return Array.Empty<MAP_HEAPALLOCENTRY>();
            }
        ...
        public unsafe MAP_THREADENTRY[] Map_GetThread(uint pid)
        {
            ...
            MAP_THREADENTRY[] m = Array.Empty<MAP_THREADENTRY>();
            if (!Vmmi.VMMDLL_Map_GetThread(hVMM, pid, out nint pMap)) { goto fail; }
        ...
        public unsafe MAP_HANDLEENTRY[] Map_GetHandle(uint pid)
        {
            ...
            MAP_HANDLEENTRY[] m = Array.Empty<MAP_HANDLEENTRY>();
            if (!Vmmi.VMMDLL_Map_GetHandle(hVMM, pid, out nint pMap)) { goto fail; }
        ...
        public unsafe MAP_NETENTRY[] Map_GetNet()
        {
            ...
            MAP_NETENTRY[] m = Array.Empty<MAP_NETENTRY>();
            if (!Vmmi.VMMDLL_Map_GetNet(hVMM, out nint pMap)) { goto fail; }
        ...
        public unsafe MAP_POOLENTRY[] Map_GetPool()
        {
            ...
            if (!Vmmi.VMMDLL_Map_GetPool(hVMM, out nint pN, 0)) { return Array.Empty<MAP_POOLENTRY>(); }
            ...
            if (nM.dwVersion != Vmmi.VMMDLL_MAP_POOL_VERSION)
            {
                ...
                return Array.Empty<MAP_POOLENTRY>();
            }
        ...
        public unsafe MAP_USERENTRY[] Map_GetUsers()
        {
            ...
            MAP_USERENTRY[] m = Array.Empty<MAP_USERENTRY>();
            if (!Vmmi.VMMDLL_Map_GetUsers(hVMM, out nint pMap)) { goto fail; }
        ...
        public unsafe MAP_SERVICEENTRY[] Map_GetServices()
        {
            ...
            MAP_SERVICEENTRY[] m = Array.Empty<MAP_SERVICEENTRY>();
            if (!Vmmi.VMMDLL_Map_GetServices(hVMM, out nint pMap)) { goto fail; }
        ...
        public unsafe MAP_PFNENTRY[] Map_GetPfn(params uint[] pfns)
        {
            ...
            if (pfns.Length == 0) { return Array.Empty<MAP_PFNENTRY>(); }
            ...
            fixed (byte* pbPfns = dataPfns)
            {
                ...
                fixed (byte* pb = new byte[cbPfns])
                {
                    ...
                    if (!result) { return Array.Empty<MAP_PFNENTRY>(); }
                    Vmmi.VMMDLL_MAP_PFN pm = Marshal.PtrToStructure<Vmmi.VMMDLL_MAP_PFN>((IntPtr)pb);
                    if (pm.dwVersion != Vmmi.VMMDLL_MAP_PFN_VERSION) { return Array.Empty<MAP_PFNENTRY>(); }
                    ...
                    for (int i = 0; i < pm.cMap; i++)
                    {
                        ...
                        {
                            dwPfn = n.dwPfn,
                            tp = (MAP_PFN_TYPE)((n._u3 >> 16) & 0x07),
                            tpExtended = (MAP_PFN_TYPEEXTENDED)n.tpExtended,
                            vaPte = n.vaPte,
        ...
        public unsafe REGISTRY_HIVE_INFORMATION[] RegHiveList()
        {
            ...
            result = Vmmi.VMMDLL_WinReg_HiveList(hVMM, null, 0, out uint cHives);
            if (!result || (cHives == 0)) { return Array.Empty<REGISTRY_HIVE_INFORMATION>(); }
            fixed (byte* pb = new byte[cHives * cbENTRY])
            {
                result = Vmmi.VMMDLL_WinReg_HiveList(hVMM, pb, cHives, out cHives);
                if (!result) { return Array.Empty<REGISTRY_HIVE_INFORMATION>(); }
                ...
                for (int i = 0; i < cHives; i++)
                {
                    ...
                    if (n.wVersion != Vmmi.VMMDLL_REGISTRY_HIVE_INFORMATION_VERSION) { return Array.Empty<REGISTRY_HIVE_INFORMATION>(); }
        ...
        public unsafe REGISTRY_ENUM RegEnum(string wszFullPathKey)
        {
            ...
            REGISTRY_ENUM re = new()
            {
                wszFullPathKey = wszFullPathKey,
                KeyList = new List<REGISTRY_KEY_ENUM>(),
                ValueList = new List<REGISTRY_VALUE_ENUM>()
            };
            fixed (byte* pb = new byte[0x1000])
            {
                i = 0;
                cchName = 0x800;
                while (Vmmi.VMMDLL_WinReg_EnumKeyExW(hVMM, wszFullPathKey, i, pb, ref cchName, out ulong ftLastWriteTime))
                {
                    REGISTRY_KEY_ENUM e = new()
                    {
                        ftLastWriteTime = ftLastWriteTime,
                        name = new string((sbyte*)pb, 0, 2 * (int)Math.Max(1, cchName) - 2, Encoding.Unicode)
                    };
                    re.KeyList.Add(e);
                    i++;
                    cchName = 0x800;
                ...
                i = 0;
                cchName = 0x800;
                while (Vmmi.VMMDLL_WinReg_EnumValueW(hVMM, wszFullPathKey, i, pb, ref cchName, out uint lpType, null, ref cbData))
                {
                    REGISTRY_VALUE_ENUM e = new()
                    {
                        type = lpType,
                        cbData = cbData,
                        name = new string((sbyte*)pb, 0, 2 * (int)Math.Max(1, cchName) - 2, Encoding.Unicode)
    ...
    public sealed class VmmScatter : IDisposable
    {
        ...
        public unsafe bool ReadStruct<T>(ulong qwA, out T result) where T : unmanaged
        {
            uint cb = (uint)sizeof(T);
            uint cbRead;
            result = default;
        ...
        public unsafe T[] ReadArray<T>(ulong qwA, uint count) where T : unmanaged
        {
            uint cb = (uint)sizeof(T) * count;
            uint cbRead;
            T[] data = new T[count];
        ...
        public unsafe bool PrepareWrite(ulong qwA, byte[] data) => PrepareWriteArray(qwA, data);

        public unsafe bool PrepareWriteArray<T>(ulong qwA, T[] data) where T : unmanaged
        {
            uint cb = (uint)sizeof(T) * (uint)data.Length;
            fixed (T* pb = data)
            {
        ...
        public unsafe bool PrepareWriteStruct<T>(ulong qwA, T value) where T : unmanaged
        {
            uint cb = (uint)sizeof(T);
            byte* pb = (byte*)&value;
    ...
    internal static partial class Vmmi
    {
        ...
        [StructLayout(LayoutKind.Sequential)]
        internal unsafe struct VMMDLL_MAP_EATENTRY
        {
            ...
            internal uint _FutureUse1;
            internal IntPtr wszFunction;
            internal IntPtr wszForwardedFunction;
        }

        internal unsafe struct VMMDLL_MAP_EAT
        {
            internal uint dwVersion;
            ...
            internal fixed uint _Reserved1[3];
